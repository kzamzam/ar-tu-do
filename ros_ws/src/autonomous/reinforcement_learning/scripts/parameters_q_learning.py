...
LASER_SAMPLE_COUNT = 1080 # was 8 which was a problem since in q_learning.launch it is 128
...
MAX_EPISODE_LENGTH = 500 #1000 #was 500 still needs tuning
...
EPS_DECAY = 8500 #was 10000 which caused very slow decay, can be fine tuned later
...
class NeuralQEstimator(nn.Module): # The neural network which will be used as a policy netowrk to decide best action
    def __init__(self):
        ...
        self.fc1 = nn.Linear(LASER_SAMPLE_COUNT, 512)
        self.fc2 = nn.Linear(512, 256)
        self.fc3 = nn.Linear(256, 64)
        self.fc4 = nn.Linear(64, ACTION_COUNT)
    ...
    def forward(self, x): # In torch forward propagation must be implemented manually
        ...
        x = F.relu(self.fc2(x))
        x = F.relu(self.fc3(x))
        return self.fc4(x) # last layer is not activated since it may be a negative value
